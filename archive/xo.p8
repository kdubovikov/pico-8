...
function _draw()
	cls()
	print(stat(0), 40,90)
	for i,c in pairs(objs.state) do
		for j,r in pairs(objs.state[i]) do
			print(objs.state[i][j], 70+10*i,70+10*j, 5)
		end
	end
...
function minmax(
	...
	for i,j in all(get_moves(state)) do
		new_state = make_move(state, i, j, player)
...
function make_move(state, mi, mj, player)
	local new_state = {}
